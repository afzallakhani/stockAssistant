...
router.get("/suggestions", async(req, res) => {
    ...
    const items = await Items.find({
            $or: [
                ...
                { itemSupplier: { $regex: q, $options: "i" } },
            ],
        })
    ...
    catchAsync(async(req, res) => {
        ...
        if (search) {
            filter.$or = [
                ...
                { itemSupplier: { $regex: search, $options: "i" } },
                { itemDescription: { $regex: search, $options: "i" } },
            ];
        }

        if (category) {
            filter.itemCategoryName = category;
        }

        if (supplier) {
            filter.itemSupplier = supplier;
        }
