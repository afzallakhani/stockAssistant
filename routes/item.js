...
//                 const monthsLeft = items[i].itemQty / dc.perMonth;
//                 items[i].stockDaysLeft = Math.floor(monthsLeft * 30);
//                 items[i].stockMonthsLeft = monthsLeft.toFixed(1);

//                 // 🔴 Status logic
//                 if (monthsLeft <= 1) {
//                     items[i].status = "Critical";
//                     criticalItems.push(items[i]);
//                 } else if (monthsLeft <= 1.5) {
//                     items[i].status = "Warning";
//                     warningItems.push(items[i]);
//                 } else {
//                     items[i].status = "Safe";
//                 }
//             } else {
...
router.get(
  ...
  catchAsync(async (req, res) => {
    ...
    for (let i = 0; i < allItemsForAlert.length; i++) {
      const dc = await getMonthlyConsumption(allItemsForAlert[i]);
...
router.get("/items/:id/supplier-stock", async (req, res) => {
  try {
    const mongoose = require("mongoose");
    const itemId = req.params.id;
    ...
    const data = await Transaction.aggregate([
      {
        $match: {
          ...
          supplier: { $ne: null }, // ignore old records
        },
      },
      {
        $group: {
          ...
          totalIn: {
            $sum: {
              $cond: [{ $in: ["$type", ["inward", "return"]] }, "$quantity", 0],
            },
          },

          totalOut: {
            $sum: {
              $cond: [{ $in: ["$type", ["outward", "lend"]] }, "$quantity", 0],
            },
          },
        },
      },
      ...
      if (lendQty > 0) {
        ...
        if (item) {
          ...
          await new Transaction({
            ...
            stockBefore,
            stockAfter,
            supplier: itemData.supplier || item.itemSupplier, // ✅ IMPORTANT
...
//         const exists = item.itemSupplier.some(
//           (s) => s.toString() === itemData.supplier,
//         );

//         if (!exists) {
//           item.itemSupplier.push(itemData.supplier);
//           await item.save();
//         }
//       }
//       if (inwardsQty > 0) {
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    for (const itemData of items) {
      ...
      if (inwardsQty > 0) {
        const item = await Items.findById(itemData.id); // ✅ FIRST

        if (item) {
          // ✅ NORMALIZE ARRAY (VERY IMPORTANT)
          if (!Array.isArray(item.itemSupplier)) {
            item.itemSupplier = item.itemSupplier ? [item.itemSupplier] : [];
          }

          // ✅ ADD SUPPLIER IF NEW
          if (itemData.supplier && itemData.supplier !== "") {
            const exists = item.itemSupplier.some(
              (s) => s.toString() === itemData.supplier,
            );
          ...
          await new Transaction({
            ...
            type: "inward",
            quantity: inwardsQty,
            supplier: itemData.supplier || item.itemSupplier, // ✅ clean
            stockBefore,
            stockAfter,
            remarks: itemData.remarks || "",
          }).save();
...
router.get(
  ...
  catchAsync(async (req, res) => {
    ...
    const transactions = await Transaction.find(filter)
      ...
      .populate("supplier", "supplierName") // ✅ ADD THIS
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(500);
