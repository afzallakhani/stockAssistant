...
router.get(
    ...
    catchAsync(async(req, res) => {
        ...
        const { supplierId, autoFill } = req.query;

        let prefillItems = [];

        if (supplierId && autoFill === "true") {
            const supplierItems = await Items.find({
                itemSupplier: supplierId,
            }).lean();
            ...
            for (let item of supplierItems) {
                ...
                if (!dc.perMonth || dc.perMonth <= 0) continue;

                const monthsLeft = item.itemQty / dc.perMonth;
                ...
                if (daysLeft <= 45) {
                    const reorderQty = Math.max(
                        0,
                        Math.ceil(dc.perMonth * 2 - item.itemQty),
                    );
